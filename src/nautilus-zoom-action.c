...
GType
nautilus_zoom_action_get_type (void)
{
	static GType type = 0;

	if (type == 0) {
		const GTypeInfo type_info = {
			sizeof (NautilusZoomActionClass),
			(GBaseInitFunc) NULL,
			(GBaseFinalizeFunc) NULL,
			(GClassInitFunc) nautilus_zoom_action_class_init,
			(GClassFinalizeFunc) NULL,
			NULL,
			sizeof (NautilusZoomAction),
			0, /* n_preallocs */
			(GInstanceInitFunc) nautilus_zoom_action_init,
		};
...
connect_proxy (GtkAction *action, GtkWidget *proxy)
{
	if (GTK_IS_TOOL_ITEM (proxy)) {
		NautilusZoomAction *naction = NAUTILUS_ZOOM_ACTION (action);

                if (GTK_WIDGET (naction->priv->window->zoom_control)->parent)
                        gtk_container_remove (GTK_CONTAINER (naction->priv->window->zoom_control->parent), naction->priv->window->zoom_control);

                gtk_container_add (GTK_CONTAINER (proxy), naction->priv->window->zoom_control);
	}
...
disconnect_proxy (GtkAction *action, GtkWidget *proxy)
{
	if (GTK_IS_TOOL_ITEM (proxy)) {
		NautilusZoomAction *naction = NAUTILUS_ZOOM_ACTION (action);
                ...
                if (naction->priv->window->zoom_control &&
                        naction->priv->window->zoom_control->parent && 
                        naction->priv->window->zoom_control->parent == proxy)
                        gtk_container_remove (GTK_CONTAINER (proxy), naction->priv->window->zoom_control);
	}
...
nautilus_zoom_action_set_property (GObject *object,
					 ...
					 GParamSpec *pspec)
{
	NautilusZoomAction *nav;

	nav = NAUTILUS_ZOOM_ACTION (object);
...
nautilus_zoom_action_class_init (NautilusZoomActionClass *class)
{
	...
	action_class->toolbar_item_type = NAUTILUS_TYPE_ZOOM_CONTROL; //GTK_TYPE_MENU_TOOL_BUTTON;
