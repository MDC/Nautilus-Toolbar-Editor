 ...
 *
 *  $Id: egg-toolbars-model.c 3240 2008-10-27 12:17:08Z chpe $
 */
  ...
  for (l1 = tl->children; l1 != NULL; l1 = l1->next)
    {
      ...
      if (toolbar->flags&EGG_TB_MODEL_NO_SAVE)
        continue;
      ...
      for (l2 = l1->children; l2 != NULL; l2 = l2->next)
	{
	  ...
	  EggToolbarsItem *item = l2->data;
          ...
          if (strcmp (item->name, "_expander") == 0)
            {
              node = xmlNewChild (tnode, NULL, (const xmlChar*) "expander", NULL);
              continue;
            }
		 ...
		 int               position)
{
  while (child)
      ...
      else if (xmlStrEqual (child->name, (const xmlChar*) "separator"))
	{
      ...
      else if (xmlStrEqual (child->name, (const xmlChar*) "expander"))
	{
          egg_toolbars_model_add_item (model, position, -1, "_expander");
	}
...
egg_toolbars_model_init (EggToolbarsModel *model)
{
  ...
  egg_toolbars_model_set_name_flags (model, "_expander", 
				     EGG_TB_MODEL_NAME_KNOWN |
				     EGG_TB_MODEL_NAME_INFINITE);
}
