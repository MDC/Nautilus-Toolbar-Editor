 ...
 *
 *  $Id: egg-toolbar-editor.c 3377 2009-01-23 23:32:40Z nshmyrev $
 */
                              ...
                              GdkDragAction     drag_action)
{
  ...
  else if (strcmp (name, "_expander") == 0)
    {
      GtkWidget *expander;
      
      expander = gtk_image_new_from_icon_name ("object-flip-horizontal", 
					      GTK_ICON_SIZE_LARGE_TOOLBAR);

      short_label = _("Flexible Space");
      item_name = g_strdup (name);
      collate_key = g_utf8_collate_key (short_label, -1);
      item = editor_create_item (editor, GTK_IMAGE (expander), 
                                 short_label, drag_action);
    }
  else
    {
